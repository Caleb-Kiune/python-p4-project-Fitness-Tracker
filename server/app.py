...
class ExerciseWorkoutResource(Resource):
    def get(self):
        ...
        exercise_workout_list = [exercise_workout.to_dict() for exercise_workout in exercise_workouts]
...
class UserWorkoutLogResource(Resource):
    def get(self):
        ...
        user_workout_log_list = [user_workout_log.to_dict() for user_workout_log in user_workout_logs]
        return make_response({"user_workout_logs": user_workout_log_list}, 200)
    ...
    def post(self):
        ...
        db.session.add(new_user_workout_log)
        db.session.commit()
        return make_response(new_user_workout_log.to_dict(), 201)

class SingleUserWorkoutLogResource(Resource):
    def get(self, user_workout_log_id):
        user_workout_log = UserWorkoutLog.query.get_or_404(user_workout_log_id)
        return make_response(user_workout_log.to_dict(), 200)

    def put(self, user_workout_log_id):
        user_workout_log = UserWorkoutLog.query.get_or_404(user_workout_log_id)
        data = request.get_json()

        user_workout_log.user_id = data.get('user_id', user_workout_log.user_id)
        user_workout_log.workout_id = data.get('workout_id', user_workout_log.workout_id)
        user_workout_log.date = data.get('date', user_workout_log.date)
        user_workout_log.notes = data.get('notes', user_workout_log.notes)

        db.session.commit()
        return make_response(user_workout_log.to_dict(), 200)

    def delete(self, user_workout_log_id):
        user_workout_log = UserWorkoutLog.query.get_or_404(user_workout_log_id)
        db.session.delete(user_workout_log)
        db.session.commit()
